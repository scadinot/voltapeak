...
> Un fichier `requirements.txt` / `pyproject.toml` verrouillant les versions est prévu dans la Vague 1 du [ROADMAP](ROADMAP.md).
...
| Fonction                                                     | Rôle                                                                   |
|--------------------------------------------------------------|------------------------------------------------------------------------|
| [`readFile`](voltapeak.py#L65)                              | Charge le fichier texte en DataFrame (encodage latin-1, 2 colonnes).   |
| [`processData`](voltapeak.py#L107)                          | Filtre, trie, inverse le signe du courant.                             |
| [`smoothSignal`](voltapeak.py#L145)                         | Lissage Savitzky-Golay avec fenêtre adaptative.                        |
| [`getPeakValue`](voltapeak.py#L191)                         | Détecte le maximum avec marge de bords + filtre de pente.              |
| [`calculateSignalBaseLine`](voltapeak.py#L249)              | Estime la ligne de base par asPLS avec exclusion autour du pic.        |
| [`plotSignalAnalysis`](voltapeak.py#L329)                   | Trace brut / lissé / baseline / corrigé / pic sur un axe matplotlib.   |
| [`processAndPlotSingleFile`](voltapeak.py#L394)             | Orchestre tout le pipeline et rafraîchit le canvas Tk.                 |
| [`launch_gui`](voltapeak.py#L490)                           | Construit la fenêtre Tk et sa boucle d'événements.                     |
| [`main`](voltapeak.py#L635)                                 | Point d'entrée — délègue à `launch_gui`.                               |
...
- **Imports inutilisés** (`re`, `platform`, `subprocess`) encore présents — à nettoyer (Vague 1).
...
- **Licence** : _à définir_ (placeholder — pas de fichier `LICENSE` à ce jour, voir Vague 1 du ROADMAP).
- **Auteur / mainteneur** : _à définir_.
- **Organisation** : GROUPE TRACE — usage interne.

Pour toute question ou contribution, contacter le mainteneur du projet.
